...
def read_voxlyze_results(objective_dict, print_log, filename="softbotsOutput.xml"):
    ...
    while (i < max_attempts) and (file_size == 0):
        try:
            file_size = os.stat(str(filename)).st_size
            this_file = open(filename, encoding='utf-8')
            this_file.close()
    ...
    results = {rank: None for rank in range(len(objective_dict))}
    for rank, details in objective_dict.items():
        this_file = open(filename, encoding='utf-8')  # TODO: is there a way to just go back to the first line without reopening the file?
        tag = details["tag"]
        if tag is not None:
...
def write_voxelyze_file(sim, env, individual, run_directory, run_name):
    ...
    voxelyze_file = open(run_directory + "/voxelyzeFiles/" + run_name + "--id_%05i.vxa" % individual.id, "w", encoding='utf-8')
